...
#warning \
	"You usb_config.h is outdated. Please use USB_PIN_DP, USB_PIN_DM and USB_PIN_DPU instead with their respective signal (no swapping!)"
...
void usb_pid_handle_setup(uint32_t addr, uint8_t *data, uint32_t endp,
			  uint32_t unused, struct rv003usb_internal *ist);
void usb_pid_handle_in(uint32_t addr, uint8_t *data, uint32_t endp,
		       uint32_t unused, struct rv003usb_internal *ist);
void usb_pid_handle_out(uint32_t addr, uint8_t *data, uint32_t endp,
			uint32_t unused, struct rv003usb_internal *ist);
void usb_pid_handle_data(uint32_t this_token, uint8_t *data,
			 uint32_t which_data, uint32_t length,
			 struct rv003usb_internal *ist);
void usb_pid_handle_ack(uint32_t dummy, uint8_t *data, uint32_t dummy2,
			uint32_t dummy3, struct rv003usb_internal *ist);

//poly_function = 0 to include CRC.
//poly_function = 2 to exclude CRC.
//This function is provided in assembly
void usb_send_data(const void *data, uint32_t length, uint32_t poly_function,
		   uint32_t token);
...
#ifdef RV003USB_OPTIMIZE_FLASH
_Static_assert((sizeof(struct usb_endpoint) == 32),
	       "usb_endpoint must be pow2 sized");
#else
_Static_assert((sizeof(struct usb_endpoint) == 16),
	       "usb_endpoint must be pow2 sized");
#endif
...
struct rv003usb_internal {
	...
	TURBO8TYPE
	setup_request; // 0 for non-setup request, 1 after setup token, is allowed to be 2 for control-out if RV003USB_SUPPORT_CONTROL_OUT is set.
